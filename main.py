...
def ingest_email(payload: schemas.EmailIngestIn, db: Session = Depends(get_db)):
    ...
    try:
        # FIX: Make the datetime offset-naive so it matches SQLAlchemy's datetime.utcnow()
        dt_timestamp = datetime.fromisoformat(payload.timestamp.replace("Z", "")).replace(tzinfo=None)
    except ValueError:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, 
    ...
    # FIX: Use datetime.utcnow() to prevent timezone offset crashes
    contact.last_contact_at = datetime.utcnow()
    db.commit()
    
    # FIX: Refresh the object to get the actual database ID generated by Postgres/SQLite
    ...
    return {
        ...
        "email_id": new_email.id if not h_results["is_spam"] else None, # We return the exact ID here
